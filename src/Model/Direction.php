...
class Direction
{
    ...
    /**
     * Direction constructeur.
     * @param string $direction
     */
    public function __construct(string $direction)
    {
        //Je passe la lettre en MAJ au cas ou erreur de saisie
        $direction = strtoupper(trim($direction));
        //Je vérifie que ma direction est correct selon les directions imposées
        if (!in_array($direction, self::AVAILABLE_DIRECTIONS)) {
            throw new InvalidArgumentException(
                sprintf("La direction peut-être seulement une des suivantes %s",
